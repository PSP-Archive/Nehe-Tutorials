//From cygwin version of Nehe tutorial lesson11
...
#include <psppower.h>	// @@@ needed by scePowerSetClockFrequency to speed up things
...
GLuint texture[1]; // @@@ int to GLuint
...
void LoadGLTextures() {	
    ...
    if (!ImageLoad("Data/tim.bmp", image1)) {
	exit(1);
    }        
...
//    glPolygonMode(GL_BACK, GL_FILL);  //@@@ remove them...
//    glPolygonMode(GL_FRONT, GL_LINE); //@@@ ...to speed up things
...
//    glPolygonMode(GL_BACK, GL_FILL);  @@@ it's an error to put it there. Need to be in InitGL
//    glPolygonMode(GL_FRONT, GL_LINE); @@@ it's an error to put it there. Need to be in InitGL

//    glBegin(GL_QUADS); moved in for/next because GL_QUADS is in pspgl internaly replaced with GL_TRIANGLE_FAN 
    for (x=0; x<44; x++) {
	    float_x  = (float) (x)/44;
...
//    glEnd(); @@@ moved in for/next boucle
...
{  
//	scePowerSetClockFrequency(333, 333, 166); // @@@	speed up things if you want...
...
//    glutFullScreen(); @@@ doesn't exist in pspgl

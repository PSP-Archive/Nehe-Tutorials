//From cygwin version of Nehe tutorial lesson 12
...
// Used keys :
// START = exit 
// PAD arrows = turn the cubes

// Known bug : in this program default light have a strange result everything seems 
//             dark unless you rotate the cubes enough. In this case there is no color 
//             applied on various cubes (lighning bug ??? or color material bug ??? or both ???)
...
//#include <unistd.h>     // needed to sleep. @@@ no need
...
// @@@ added because pspgl seems to have light initialised before use
...
void LoadGLTextures() {	
    ...
    if (!ImageLoad("Data/cube.bmp", image1)) {
	exit(1);
    }        
...
//    glEnable(GL_LIGHT0); @@@ in this program default light have a strange result everything seems dark unless you...
//    glEnable(GL_LIGHTING); @@@ ...rotate the cubes enough. In this case there is no color applied on various cubes (lighning bug ??? or color material bug ??? or both ???)
...
//    glutFullScreen(); @@@ don't exist in pspgl
